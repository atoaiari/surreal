...
from datageneration.utils.blender_utils import *
...
def main():
    ...
    parser.add_argument('--textures', type=int, default=10,
    ...
    log_message("number of textures: %d" % n_textures)
    ...
    logs_path = join(output_path, "logs")
    ...
    print("LOG PATH: %s/%s.log"%(logs_path, os.path.basename(output_path)))
    import logging 
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("%s/%s.log"%(logs_path, os.path.basename(output_path)), mode='a', encoding='utf-8', delay=False)
        ]
    )
    ...
    # set orientations
    orientations = list(np.arange(0, 360, (360/n_orientations)))
    ...
    # cloth_img_name_choice = ["textures/female/nongrey_female_0902.jpg", "textures/male/nongrey_male_0388.jpg"]
    cloth_img_name_choice = ["textures/female/nongrey_female_0257.jpg", "textures/male/nongrey_male_0388.jpg"]
    # cloth_img_name_choice = ["textures/female/nongrey_female_0257.jpg", "textures/female/nongrey_female_0257.jpg"]
    # cloth_img_name_choice = []
    ...
    #     cloth_img_name_choice.extend(np.random.choice(clothing_txt_paths, size=n_textures, replace=False))
    
    img_ct = 0
    for igndr, gndr in enumerate(gender):
        # fixed clothing texture
        cloth_img_name = join(smpl_data_folder, cloth_img_name_choice[igndr])
        ...
        # shapes = [np.array([ 2.25176191, -3.7883464 ,  0.46747496,  3.89178988,  2.20098416,  0.26102114, -3.07428093,  0.55708514, -3.94442258, -2.88552087]), #fat
        #             np.array([-2.26781107,  0.88158132, -0.93788176, -0.23480508,  1.17088298,  1.55550789,  0.44383225,  0.37688275, -0.27983086,  1.77102953]), #thin
        #             np.array([ 0.00404852,  0.8084637 ,  0.32332591, -1.33163664,  1.05008727,  1.60955275,  0.22372946, -0.10738459,  0.89456312, -1.22231216]), #short
        #             np.array([ 3.63453289,  1.20836171,  3.15674431, -0.78646793, -1.93847355, -0.32129994, -0.97771656,  0.94531640,  0.52825811, -0.99324327])] #tall
        if igndr == 0:      # use only the shapes extracted from the female model
            shapes_idx = np.random.choice(np.arange(len(fshapes)), size=n_shapes, replace=False)
            shapes = fshapes[shapes_idx]
        ndofs = 10
            ...
            for iorientation, orientation in enumerate(orientations):
                for background in backgrounds:        
                    ...
                    for iframe, (seq_frame, pose, trans) in enumerate(zip(frames, data['poses'][frames], translations)):
                        ...
                        dict_info['img_path'] = '%04d-%s-%04d-%s-%s-%d-%d.png' % (img_ct, name.replace(" ", ""), get_real_frame(seq_frame), gndr, os.path.splitext(os.path.basename(background))[0], iorientation, ishape)
                        scene.render.filepath = join(output_path, dict_info['img_path'])

                        log_message("Rendering image %d: frame %d with: gender %s, shape %d, background %s, orientation %.2f" % (img_ct, seq_frame, gndr, ishape, os.path.splitext(os.path.basename(background))[0], orientation))
